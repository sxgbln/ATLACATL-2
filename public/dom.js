...
function postData() {
  ...
  async function sendPostRequest() {
    try {
      const fetchResponse = await fetch(`${BASE_API_URL}/`, requestOptions); // Use BASE_API_URL
...
function postComment() {
  ...
  async function sendCommentRequest() {
    try {
      const fetchResponse = await fetch(`${BASE_API_URL}/server/comment`, requestOptions); // Use BASE_API_URL
      const responseText = await fetchResponse.text();
...
function getRecordsAsc() {
  ...
  async function asyncGetAsc() {
    try {
      const fetchResponse = await fetch(`${BASE_API_URL}/server/get/sortasc`); // Use BASE_API_URL
      if (!fetchResponse.ok) {
...
function getRecordsDesc() {
  ...
  async function asyncGetDesc() {
    try {
      const fetchResponse = await fetch(`${BASE_API_URL}/server/get/sortdesc`); // Use BASE_API_URL
      webConsole.value = `HTTP Status: ${fetchResponse.status}`; // Display status on console
...
function viewCardComments(cardId) {
  ...
  async function fetchCardAndComments() {
    try {
      const fetchResponse = await fetch(`${BASE_API_URL}/server/card/${cardId}`); // Use BASE_API_URL
      if (!fetchResponse.ok) {
        throw new Error(`HTTP error! status: ${fetchResponse.status}`);
      }
...
function handleLike(cardId) {
  ...
  async function sendLikeRequest() {
    try {
      const fetchResponse = await fetch(`${BASE_API_URL}/server/like`, requestOptions); // Use BASE_API_URL
